...
// Sample data for each car category
const carsData = {
  expo: {
    title: "Expo",
    subtitle: "Tata Nano",
...
// Grabbing the DOM elements we need to update
...
// Function to load data for a given car category
function loadCarData(carKey) {
  ...
  carFeatures.innerHTML = ""; // Clear existing items
  ...
  // Update sub images
  carSubImages.innerHTML = ""; // Clear existing
  // Create a 2x2 grid by using row + col-6
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  categoryButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // Get the car key from data attribute
      const carKey = btn.getAttribute("data-car");
      loadCarData(carKey);
    });
  });

  // Optionally, load a default car on page load (e.g., "expo")
  // loadCarData("expo");
});